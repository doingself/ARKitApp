...
class ThreeARViewController: UIViewController {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        //设置节点的位置
        node.position = SCNVector3(x: 0, y: -1, z: -15)
        //缩放
        node.scale = SCNVector3Make(0.1, 0.1, 0.1);
        
        for childNode in node.childNodes{
            childNode.position = SCNVector3(x: 0, y: -1, z: -1)
            childNode.scale = SCNVector3Make(0.1, 0.1, 0.1);
        }
...
extension ThreeARViewController: ARSessionDelegate{
    // MARK: AR Session 代理
...
//        print("ARSessionDelegate 相机移动 frame = \(frame)")
        ...
        planeNode.position = SCNVector3(x: col.x, y: col.y, z: col.z)
...
extension ThreeARViewController: ARSCNViewDelegate{
    // MARK: AR SCNView 代理
